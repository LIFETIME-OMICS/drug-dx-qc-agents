...
The pipeline ensures **quality control** by detecting medications that lack corresponding diagnoses in patient records. The pipeline can also be used to generate **digital twin patients** with prescribed drug classes and linked diagnoses without exposing medication details and any original data. 

Moreover, the project compares two paradigms: one that mimics how humans use software tools and reason about their outputs, and another that shows what happens when an autonomous agent does all pipeline tasks internally. This lets us evaluate whether the transparency and modularity of the pipeline outweigh the speed and adaptability of the autonomous approach.
...
2. **drug-classifier**  
   ...
   Output: atc_database.json and drug_classifications.csv generated by agent in agents\drug_classifier.py 
...
3. **qc-evaluator**  
   LLM mapping to ICD-10 diagnosis (dx) from `medication.csv` and compares dx against EHR `condition.csv` or `diagnosis.csv` and flags patients with prescriptions lacking corresponding diagnoses.  
   Output: Text string output generated by agent in agents\qc_evaluator.py   
...
**Demo Script to run first 2 agents in sequence to create the local ATC database**   
scripts\build_atc_database.py

**Demo Script to test the 2 pipelines**   
scripts\demo_qc_evaluator.py
...
│   ├── demo_qc_evaluator.py
│   ├── demo_stats_summarizer.py
│
│── tests/
│   ├── conftest.py
│   ├── test_drug_classifier.py
│   ├── test_drug_identifier.py
│
│── main.py
│── requirements.txt
│── README.md
...
5. **Synthetic data for testing**: https://www.kaggle.com/datasets/mexwell/synthetic-al-medical-records/data 
